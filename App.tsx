...
function App() {
  ...
  useEffect(() => {
    try {
      ...
      // ✅ FIX: Créer le Service Worker en inline pour éviter l'erreur MIME type
      if ('serviceWorker' in navigator) {
        // Service Worker inline (évite problème MIME type)
        const swCode = `
              ...
              if (firebaseConfig && firebaseConfig.apiKey) {
                try {
                  ...
                  messaging.onBackgroundMessage((payload) => {
                    ...
                    const title = payload.notification?.title || 'SmartCabb';
                    const options = {
          ...
          // Gérer les clics sur notification
          self.addEventListener('notificationclick', (event) => {
            event.notification.close();
            event.waitUntil(
              clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clientList) => {
                for (const client of clientList) {
                  if (client.url.includes('smartcabb') && 'focus' in client) {
                    return client.focus();
                  }
                }
        ...
        // Créer blob URL pour le Service Worker
        const blob = new Blob([swCode], { type: 'application/javascript' });
        const swUrl = URL.createObjectURL(blob);

        navigator.serviceWorker.register(swUrl)
          .then((registration) => {
            console.log('✅ Service Worker Firebase enregistré (inline):', registration.scope);
