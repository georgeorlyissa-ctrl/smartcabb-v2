...
function DriverAppContent() {
  ...
  // ✅ FCM : Initialiser FCM pour recevoir les notifications
  useEffect(() => {
    ...
    setupFCMForUser(state.currentDriver.id, 'driver', (payload) => {
      ...
      switch (data.type) {
        case 'new_ride_request':
          ...
          // ✅ CRITIQUE : Déclencher un événement personnalisé pour DriverDashboardNew
          const event = new CustomEvent('fcm-new-ride-request', {
            detail: {
              rideId: data.rideId,
              pickup: {
                latitude: parseFloat(data.pickupLat),
                longitude: parseFloat(data.pickupLng),
                address: data.pickupName
              },
              dropoff: {
                latitude: parseFloat(data.destinationLat),
                longitude: parseFloat(data.destinationLng),
          ...
          console.log('✅ Événement fcm-new-ride-request dispatché');
          break;

        case 'ride_cancelled':
          console.log('❌ Course annulée !', { title, body });
          // TODO : Afficher notification d'annulation
          break;

        case 'payment_received':
          console.log('💰 Paiement reçu !', { title, body });
          // TODO : Afficher notification de paiement
          break;
    ...
    return () => {
      console.log('🛑 Nettoyage FCM conducteur');
    };
