#!/usr/bin/env node

/**
 * 🔧 GÉNÉRATION AUTOMATIQUE DU FICHIER info.tsx
 * 
 * Ce script génère /utils/supabase/info.tsx depuis les variables d'environnement
 * Il s'exécute AVANT le build Vite pour garantir la disponibilité des clés
 * 
 * IMPORTANT: Extension .cjs car le projet utilise "type": "module"
 */

const fs = require('fs');
const path = require('path');

console.log('🔧 Génération du fichier info.tsx...');

// Récupérer les variables d'environnement
const projectId = process.env.VITE_SUPABASE_PROJECT_ID || process.env.SUPABASE_PROJECT_ID;
const publicAnonKey = process.env.VITE_SUPABASE_ANON_KEY || process.env.SUPABASE_ANON_KEY;

console.log('📋 Variables d\'environnement détectées:');
console.log('   VITE_SUPABASE_PROJECT_ID:', process.env.VITE_SUPABASE_PROJECT_ID ? '✅' : '❌');
console.log('   SUPABASE_PROJECT_ID:', process.env.SUPABASE_PROJECT_ID ? '✅' : '❌');
console.log('   VITE_SUPABASE_ANON_KEY:', process.env.VITE_SUPABASE_ANON_KEY ? '✅' : '❌');
console.log('   SUPABASE_ANON_KEY:', process.env.SUPABASE_ANON_KEY ? '✅' : '❌');

// Validation
if (!projectId) {
  console.error('');
  console.error('❌ ERREUR CRITIQUE: Project ID manquant !');
  console.error('');
  console.error('   Veuillez définir une de ces variables d\'environnement:');
  console.error('   - VITE_SUPABASE_PROJECT_ID');
  console.error('   - SUPABASE_PROJECT_ID');
  console.error('');
  console.error('   Valeur attendue: zaerjqchzqmcxqblkfkg');
  console.error('');
  process.exit(1);
}

if (!publicAnonKey) {
  console.error('');
  console.error('❌ ERREUR CRITIQUE: Anon Key manquante !');
  console.error('');
  console.error('   Veuillez définir une de ces variables d\'environnement:');
  console.error('   - VITE_SUPABASE_ANON_KEY');
  console.error('   - SUPABASE_ANON_KEY');
  console.error('');
  process.exit(1);
}

// Créer le répertoire si nécessaire
const dir = path.join(__dirname, '..', 'utils', 'supabase');
if (!fs.existsSync(dir)) {
  console.log('📁 Création du répertoire utils/supabase...');
  fs.mkdirSync(dir, { recursive: true });
}

// Générer le contenu du fichier
const content = `/* AUTOGENERATED FILE - DO NOT EDIT CONTENTS */

export const projectId = "${projectId}"
export const publicAnonKey = "${publicAnonKey}"
`;

// Écrire le fichier
const filePath = path.join(dir, 'info.tsx');
fs.writeFileSync(filePath, content, 'utf-8');

console.log('');
console.log('✅ Fichier /utils/supabase/info.tsx généré avec succès !');
console.log('   Project ID:', projectId);
console.log('   Anon Key:', publicAnonKey.substring(0, 30) + '...');
console.log('');
